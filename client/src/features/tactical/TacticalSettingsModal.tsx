...
const SETTINGS_TABS = [
  {
    ...
    label: "AI Core",
    icon: BrainCircuit,
    desc: "Model configuration & connections",
  },
  {
    id: "mcp",
    label: "MCP Plugins",
    icon: Plug,
    desc: "Machine control protocol servers",
  },
  {
    id: "skills",
    label: "Skills Array",
    icon: Wrench,
    desc: "Backend and custom skill definitions",
  },
  {
    id: "system",
    label: "System",
    icon: Settings,
    desc: "Tactical OS preferences",
  },
] as const;
...
function ModelConfigSection({
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div>
        <h2 className="font-mono text-lg text-cyan-50">AI Core Intelligence</h2>
        <p className="mt-1 text-xs text-slate-400">
          Configure the primary LLM engine driving the tactical advisory system.
        </p>
      </div>
        ...
        <div className="space-y-4 rounded-xl border border-cyan-400/10 bg-cyan-950/5 p-5 shadow-inner">
          ...
          <div className="pt-2">
            ...
            >
              ...
              {checkingModel ? "VERIFYING LINK..." : "TEST CONNECTION"}
            </Button>
          </div>
        </div>
      </div>
      ...
      {modelCheckResult && (
        ...
        >
          ...
          <div className="mt-3 grid grid-cols-3 gap-4 font-mono text-xs opacity-80">
            <div>
              <span className="block text-[9px] text-slate-400">ENDPOINT</span>
              {modelCheckResult.endpoint || "N/A"}
            </div>
            <div>
              <span className="block text-[9px] text-slate-400">
                AUTH STATUS
              </span>
              {modelCheckResult.auth_ok ? "VERIFIED" : "FAILED"}
            </div>
            <div>
              <span className="block text-[9px] text-slate-400">
                MODEL LOOKUP
              </span>
              {modelCheckResult.models_listed ? "VERIFIED" : "FAILED"}
            </div>
          </div>
      ...
      <div className="rounded-lg border border-cyan-400/10 bg-cyan-950/10 p-3 font-mono text-[10px] text-cyan-400/60 flex items-start gap-2">
        ...
        <p>
          Models are configured securely and passed via HTTP headers
          (X-AICC-Model-*) to the backend runtime. Leaving fields empty will
...
function McpConfigSection({
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div>
        <h2 className="font-mono text-lg text-cyan-50">MCP Plugins Array</h2>
        <p className="mt-1 text-xs text-slate-400">
          Manage Machine Control Protocol (MCP) bridges to extend AI tactical
          capabilities.
        </p>
      </div>
      ...
      <div className="flex items-center justify-between rounded-xl border border-cyan-400/10 bg-cyan-950/5 p-4 shadow-inner">
        <div>
          <div className="font-mono text-xs font-bold text-slate-200">
            Built-in Project MCP
          </div>
          <div className="mt-1 text-[10px] text-slate-500">
            Enable local workspace inspection tools
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            {projectMcpEnabled ? "ONLINE" : "OFFLINE"}
          </span>
      ...
      <McpSection title="External MCP Nodes" count={activeMcpServersCount}>
        <div className="space-y-3">
          {mcpServers.length === 0 ? (
      ...
      <div className="rounded-xl border border-cyan-400/10 bg-cyan-950/5 p-5 shadow-inner">
        ...
        <div className="grid gap-4 md:grid-cols-[1fr_2fr_100px_auto]">
          <input
            ...
            placeholder="Node Name (e.g. weather)"
            value={newServerName}
          />
          <input
            className={INPUT_CLASS}
            onChange={(e) => onNewServerEndpointChange(e.target.value)}
            placeholder="Endpoint / Command"
            value={newServerEndpoint}
          />
          <select
            className={SELECT_CLASS}
            onChange={(e) => onNewServerTransportChange(e.target.value as any)}
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="font-mono text-lg text-cyan-50">
            Skill Array Management
          </h2>
          <p className="mt-1 text-xs text-slate-400">
            Define and monitor AI native behaviors and actionable skills.
          </p>
        </div>
        <Button
      ...
      <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-cyan-900/50">
              {registeredSkills.length === 0 ? (
                <div className="flex h-20 items-center justify-center rounded-lg border border-dashed border-cyan-400/20 bg-cyan-950/5 font-mono text-[10px] text-cyan-600/50">
                  {skillsLoading ? "SYNCING..." : "NO SKILLS FOUND"}
                </div>
              ) : (
        ...
        {/* Custom Skills */}
        <div className="space-y-4">
          <McpSection title="Custom Injections" count={activeCustomSkillsCount}>
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-cyan-900/50">
...
function SystemConfigSection({ onClearMessages }: TacticalSettingsProps) {
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div>
        <h2 className="font-mono text-lg text-cyan-50">
          Tactical System Operations
        </h2>
        <p className="mt-1 text-xs text-slate-400">
          Global preferences and critical operations for the AICC Command
          runtime.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-red-500/20 bg-red-950/10 p-6">
        ...
        <p className="mt-2 text-xs text-slate-400">
          These actions are irreversible and will affect the current tactical
          session.
        </p>
        ...
        <div className="mt-6 flex items-center justify-between rounded-lg border border-red-500/10 bg-black/20 p-4">
          <div>
            ...
            <div className="mt-1 text-[10px] text-slate-500">
              Clears all chat history and local AI context for the current
              scenario.
            </div>
          </div>
          <Button
          ...
          >
            <Trash2 className="size-4 mr-2" /> PURGE
          </Button>
        </div>
      </div>
    </div>
