...
describe("Scenario", () => {
  ...
  test("gets targets hostile to the querying side", () => {
    const [testScenario, testAirbase, testAircraft, testFacility, testShip] =
      getTestUnits();

    testScenario.relationships.addHostile("BLUE", "RED");

    const blueEnemyTargets =
      testScenario.getAllTargetsFromEnemySides("BLUE");
    expect(blueEnemyTargets).toContain(testFacility);
    expect(blueEnemyTargets).not.toContain(testAircraft);
    expect(blueEnemyTargets).not.toContain(testAirbase);
    expect(blueEnemyTargets).not.toContain(testShip);

    const redEnemyTargets = testScenario.getAllTargetsFromEnemySides("RED");
    expect(redEnemyTargets).toContain(testAircraft);
    expect(redEnemyTargets).toContain(testAirbase);
    expect(redEnemyTargets).toContain(testShip);
    expect(redEnemyTargets).not.toContain(testFacility);
  });

  test("clamps aircraft weapon quantity to its maximum", () => {
    const [testScenario, , testAircraft] = getTestUnits();

    testScenario.updateAircraftWeaponQuantity(
      testAircraft.id,
      testAircraft.weapons[0].id,
      10
    );

    expect(testAircraft.weapons[0].currentQuantity).toBe(
      testAircraft.weapons[0].maxQuantity
    );
  });
});
