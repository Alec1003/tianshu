...
export default function ScenarioMap({
  ...
  const [aircraftLayer] = useState(() => new AircraftLayer(projection, 3));
  const [airbasesLayer] = useState(() => new AirbasesLayer(projection, 1));
  const [facilityLayer] = useState(() => new FacilityLayer(projection, 1));
  const [threatRangeLayer] = useState(() => new ThreatRangeLayer(projection));
  const [aircraftRouteLayer] = useState(
    () => new RouteLayer("aircraftRouteLayer", projection)
  );
  const [shipRouteLayer] = useState(
    () => new RouteLayer("shipRouteLayer", projection)
  );
  const [weaponLayer] = useState(() => new WeaponLayer(projection, 2));
  const [shipLayer] = useState(() => new ShipLayer(projection, 1));
  const [referencePointLayer] = useState(
    () => new ReferencePointLayer(projection, 1)
  );
  const [featureLabelLayer] = useState(
  ...
  const routeMeasurementTooltipElementRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    ...
    const clickKey = theMap.on("click", (event) => {
    ...
    theMap.getViewport().addEventListener("contextmenu", contextMenuHandler);
    ...
    return () => {
      theMap.getViewport().removeEventListener("contextmenu", contextMenuHandler);
      unByKey(pointerMoveKey);
      unByKey(moveEndKey);
      unByKey(clickKey);
      theMap.setTarget();
    };
  }, [game, setCurrentMouseMapCoordinatesToContext, setCurrentScenarioSidesToContext, setCurrentScenarioTimeToContext, theMap]);
  ...
  function getMapClickContext(event: MapBrowserEvent<MouseEvent>): string {
    ...
    if (
      selectedFeatureType === "aircraft" &&
      routeMeasurementDrawLineRef.current
    ) {
      context = "moveAircraft";
    } else if (
      selectedFeatureType === "ship" &&
      routeMeasurementDrawLineRef.current
    ) {
      context = "moveShip";
  ...
  function drawNextFrame(observation: Scenario) {
    const aircraftCountChanged =
      aircraftLayer.featureCount !== observation.aircraft.length;
    if (aircraftCountChanged) {
      aircraftLayer.refresh(observation.aircraft);
    } else {
      aircraftLayer.updateAircraftFeatures(observation.aircraft);
    }

    if (weaponLayer.featureCount !== observation.weapons.length) {
      weaponLayer.refresh(observation.weapons);
    } else {
      weaponLayer.updateWeaponFeatures(observation.weapons);
    }

    const shipCountChanged = shipLayer.featureCount !== observation.ships.length;
    if (shipCountChanged) {
      shipLayer.refresh(observation.ships);
    } else {
      shipLayer.updateShipFeatures(observation.ships);
    }

    refreshRouteLayer(observation);
    if (featureLabelVisible) {
      if (aircraftCountChanged || shipCountChanged) {
        refreshFeatureLabelLayer();
      } else {
        featureLabelLayer.updateFeatureLabelGeometry(observation.aircraft);
        featureLabelLayer.updateFeatureLabelGeometry(observation.ships);
      }
    }
    ...
    if (threatRangeVisible) {
      const threatEntities = [...observation.facilities, ...observation.ships];
      if (threatRangeLayer.featureCount !== threatEntities.length) {
        threatRangeLayer.refresh(threatEntities);
      } else {
        threatRangeLayer.updateRangeFeatures(threatEntities);
      }
    }
    if (
    ...
    } else if (featureLabelVisible) {
      featureLabelLayer.updateFeatureLabelGeometry(observation.referencePoints);
    }
  }
  ...
  function refreshRouteLayer(observation: Scenario) {
    ...
    aircraftRouteLayer.syncRouteFeatures(observation.aircraft);
    shipRouteLayer.syncRouteFeatures(observation.ships);
  }
  ...
  function createRouteMeasurementTooltip() {
    ...
    routeMeasurementTooltipElementRef.current.className =
  ...
  function cleanUpRouteDrawLineAndMeasurementTooltip() {
    ...
    if (routeMeasurementTooltipRef.current) {
      theMap.removeOverlay(routeMeasurementTooltipRef.current);
    }
    ...
    if (routeMeasurementListenerRef.current) {
      unByKey(routeMeasurementListenerRef.current);
    }
    routeMeasurementListenerRef.current = undefined;
  }
      ...
      routeMeasurementListenerRef.current = drawLineFeature
        ...
        ?.on("change", function (event) {
          ...
          if (routeMeasurementTooltipElementRef.current) {
            ...
            routeMeasurementTooltipElementRef.current.style.fontWeight =
              "bold";
          }
  ...
  function applyScenarioFromAI(scenario: Record<string, unknown>) {
    try {
      ...
      loadFeatureEntitiesState();
      refreshAllLayers();
      toastContext?.addToast("AI command applied to scenario.", "success");
    } catch (_error) {
      toastContext?.addToast("Failed to apply AI scenario update.", "error");
    }
  }
  ...
  return (
    <>
      ...
      {missionEditorActive.open &&
        game.currentScenario.missions.filter(
          ...
          <MissionEditorCard
            ...
            aircraft={game.currentScenario.aircraft.filter((aircraft) => {
              if (aircraft.sideId !== game.currentSideId) return false;
              const occupiedByOtherMission = game.currentScenario.missions.some(
                (mission) =>
                  mission.id !== missionEditorActive.selectedMissionId &&
                  mission.assignedUnitIds.includes(aircraft.id)
              );
              return !occupiedByOtherMission;
            })}
            referencePoints={game.currentScenario.referencePoints.filter(
              (referencePoint) => referencePoint.sideId === game.currentSideId
            )}
      ...
      <OpenClawAssistantPanel
        mobileView={mobileView}
        onApplyScenario={applyScenarioFromAI}
      />
