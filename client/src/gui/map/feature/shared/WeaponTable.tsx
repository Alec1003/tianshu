...
export default function WeaponTable(props: Readonly<WeaponTableProps>) {
  ...
  useEffect(() => {
    setUnitWeapons(props.unitWithWeapon.weapons);
  }, [props.unitWithWeapon]);
        ...
        <Table size="small" aria-label="unitWithWeapon-weapons-table">
          <TableHead>
            <TableRow sx={tableRowStyle}>
              ...
              >
                <Tooltip title={t("weaponTable.add")}>
                  <IconButton
              ...
              unitWeapons.map((weapon, index) => (
                ...
                >
                  <TableCell align="right" sx={tableValueCellStyle}>
                    {localizeWeaponName(weapon.className)}
                  </TableCell>
                  <TableCell align="right" sx={tableValueCellStyle}>
                  ...
                  <TableCell align="center" sx={tableValueCellStyle}>
                    <>
                      <Tooltip title={t("weaponTable.decrease")}>
                        <IconButton
                          onClick={() =>
                      ...
                      <Tooltip title={t("weaponTable.increase")}>
                        <IconButton
                          onClick={() =>
                            _handleUpdateWeaponQuantity(weapon.id, 1)
                          }
                  ...
                  <TableCell align="right" sx={tableValueCellStyle}>
                    <>
                      {props.handleUnitAttack && (
                        <Tooltip title={t("weaponTable.launch")}>
                          <IconButton
                            onClick={(
                              event: React.MouseEvent<HTMLButtonElement>
                            ) => {
                      ...
                      <Tooltip title={t("weaponTable.delete")}>
                        <IconButton
            ...
            title={
              <Stack direction={"column"} spacing={0.1}>
                <Typography variant="body2" sx={{ fontWeight: "bold" }}>
                  {t("weaponTable.tooltipSpeed", {
                    value: weapon.speed.toFixed(0),
                  })}
                </Typography>
                <Typography variant="body2" sx={{ fontWeight: "bold" }}>
                  {t("weaponTable.tooltipMaxFuel", {
                    value: weapon.maxFuel.toFixed(2),
                  })}
                </Typography>
                <Typography variant="body2" sx={{ fontWeight: "bold" }}>
                  {t("weaponTable.tooltipFuelRate", {
                    value: weapon.fuelRate.toFixed(2),
                  })}
                </Typography>
                <Typography variant="body2" sx={{ fontWeight: "bold" }}>
                  {t("weaponTable.tooltipLethality", {
                    value: (weapon.lethality * 100).toFixed(2),
                  })}
                </Typography>
              </Stack>
            }
