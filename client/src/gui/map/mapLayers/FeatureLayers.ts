...
import { localizeWeaponName } from "@/i18n/entityNames";
...
class FeatureLayer {
  ...
  hasFeatureById(id: string) {
    return Boolean(this.layerSource.getFeatureById(id));
  }
}
...
export class AircraftLayer extends FeatureLayer {
  ...
  updateAircraftFeatures(aircraftList: Aircraft[]) {
    this.updateAircraftGeometry(aircraftList);
    aircraftList.forEach((aircraft) => {
      const feature = this.layerSource.getFeatureById(aircraft.id);
      if (feature) {
        feature.set("selected", aircraft.selected);
        feature.set("heading", aircraft.heading);
      }
    });
  }
...
export class ThreatRangeLayer extends FeatureLayer {
  ...
  updateRangeFeatures(entities: GameEntityWithRange[]) {
    entities.forEach((entity) => {
      const feature = this.layerSource.getFeatureById(entity.id);
      if (feature) {
        feature.setGeometry(
          new Circle(
            fromLonLat([entity.longitude, entity.latitude], this.projection),
...
export class RouteLayer extends FeatureLayer {
  ...
  syncRouteFeatures(moveableUnitList: GameEntityWithRoute[]) {
    const activeRouteIds = new Set<string>();

    moveableUnitList.forEach((moveableUnit) => {
      if (moveableUnit.route.length === 0) {
        this.removeFeatureById(moveableUnit.id);
        return;
      }

      activeRouteIds.add(moveableUnit.id);
      if (this.hasFeatureById(moveableUnit.id)) {
        this.updateRouteFeature(moveableUnit);
      } else {
        this.addRouteFeature(moveableUnit);
      }
    });
    ...
    this.layerSource
      ...
      .filter((id) => !activeRouteIds.has(id))
      .forEach((id) => this.removeFeatureById(id));
  }
}
...
export class WeaponLayer extends FeatureLayer {
  ...
  updateWeaponFeatures(weaponList: Weapon[]) {
    weaponList.forEach((weapon) => {
      const feature = this.layerSource.getFeatureById(weapon.id);
      if (feature) {
        feature.setGeometry(
          new Point(
            fromLonLat([weapon.longitude, weapon.latitude], this.projection)
        ...
        feature.set("heading", weapon.heading);
        feature.set("name", localizeWeaponName(weapon.name));
      }
    });
  }
}
...
export class FeatureLabelLayer extends FeatureLayer {
  ...
  updateFeatureLabelGeometry(entities: GameEntity[]) {
    entities.forEach((entity) => {
      const feature = this.layerSource.getFeatureById(entity.id);
      if (feature) {
        feature.setGeometry(
...
export class ShipLayer extends FeatureLayer {
  ...
  updateShipGeometry(shipsList: Ship[]) {
  ...
  updateShipFeatures(shipsList: Ship[]) {
    this.updateShipGeometry(shipsList);
    shipsList.forEach((ship) => {
      const feature = this.layerSource.getFeatureById(ship.id);
      if (feature) {
        feature.set("selected", ship.selected);
        feature.set("heading", ship.heading);
      }
    });
  }
}
