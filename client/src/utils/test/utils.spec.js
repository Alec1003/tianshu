...
import { unixToLocalDateTime, unixToLocalTime } from "../dateTimeFunctions";
...
describe("testing distance math functions", () => {
  ...
  it.each([
    ...
    [
      ...
      100,
      90,
      { lat: 29.99592526877547, lon: 121.03841871861648 },
    ],
  ])(
...
describe("testing miscellaneous utility functions", () => {
  ...
  test("local date time includes date and time", () => {
    ...
    expect(formattedDateTime).toMatch(/\d{2}:\d{2}:\d{2}/);
    expect(formattedDateTime).not.toBe(unixToLocalTime(1699073110));
  });
