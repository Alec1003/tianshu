"""攻击路径规划（自由模式）：每对 platform-target 由用户指定 mode.

用法:
    python algorithms/route_attack.py -c scenario.json -o result.json
"""
...
def run(scenario: dict[str, Any], output_path: str = "output/route_attack_result.json") -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        completed = subprocess.run(
            [sys.executable, "-m", "src.route_plan.attack", str(scenario_path), "-o", str(output_dir)],
            cwd=str(BASE_DIR),
            capture_output=True,
            text=True,
            encoding="utf-8",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="攻击路径规划（自由模式）")
    parser.add_argument("-c", "--config", required=True, help="场景配置 JSON 文件路径")
    parser.add_argument("-o", "--output", default="output/route_attack_result.json", help="结果输出路径")
