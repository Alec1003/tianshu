"""攻击路径规划 — Mode 1：从起点向目标终点直接规划路径.

用法:
    python algorithms/route_mode1.py -c scenario.json -o result.json
"""
...
def run(scenario: dict[str, Any], output_path: str = "output/route_mode1_result.json") -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        completed = subprocess.run(
            [sys.executable, "-m", "src.route_plan.attack1", str(scenario_path), "-o", str(output_dir)],
            cwd=str(BASE_DIR),
            capture_output=True,
            text=True,
            encoding="utf-8",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="攻击路径规划 — Mode 1")
    parser.add_argument("-c", "--config", required=True, help="场景配置 JSON 文件路径")
    parser.add_argument("-o", "--output", default="output/route_mode1_result.json", help="结果输出路径")
