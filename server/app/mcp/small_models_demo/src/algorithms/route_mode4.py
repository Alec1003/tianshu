"""攻击路径规划 — Mode 4：先生成攻击点候选，再经过 waypoint 集结点.

用法:
    python algorithms/route_mode4.py -c scenario.json -o result.json
"""
...
def run(scenario: dict[str, Any], output_path: str = "output/route_mode4_result.json") -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        completed = subprocess.run(
            [sys.executable, "-m", "src.route_plan.attack4", str(scenario_path), "-o", str(output_dir)],
            cwd=str(BASE_DIR),
            capture_output=True,
            text=True,
            encoding="utf-8",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="攻击路径规划 — Mode 4")
    parser.add_argument("-c", "--config", required=True, help="场景配置 JSON 文件路径")
    parser.add_argument("-o", "--output", default="output/route_mode4_result.json", help="结果输出路径")
