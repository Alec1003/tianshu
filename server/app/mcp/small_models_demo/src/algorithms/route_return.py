...
用法:
    python algorithms/route_return.py -c scenario.json -o result.json
"""
...
def run(scenario: dict[str, Any], output_path: str = "output/route_return_result.json") -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        completed = subprocess.run(
            [sys.executable, "-m", "src.route_plan.return_plan", str(scenario_path), "-o", str(output_dir)],
            cwd=str(BASE_DIR),
            capture_output=True,
            text=True,
            encoding="utf-8",
        ...
        result = {
            ...
            "paths_result": json.loads(paths_json.read_text(encoding="utf-8")) if paths_json.exists() else {},
...
def main() -> None:
    ...
    parser.add_argument("-o", "--output", default="output/route_return_result.json", help="结果输出路径")
