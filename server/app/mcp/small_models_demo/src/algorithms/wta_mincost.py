"""WTA 最小花费分配：两波次标准置信度 (waves=2, confidence=0.95).

用法:
    python algorithms/wta_mincost.py -c config.json -o result.json
"""
...
def run(config: dict[str, Any], mc_trials: int = 5000, output_path: str = "output/wta_mincost_result.json") -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        subprocess.run(
            [sys.executable, "-m", "wta.allocator_mincost", "--config", str(scenario_path), "--output", str(output_dir), "--mc-trials", str(mc_trials)],
            cwd=str(BASE_DIR),
            capture_output=True,
            text=True,
            encoding="utf-8",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="WTA 最小花费分配")
    parser.add_argument("-c", "--config", required=True, help="场景配置 JSON/YAML 文件路径")
    parser.add_argument("-o", "--output", default="output/wta_mincost_result.json", help="结果输出路径")
