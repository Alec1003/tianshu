"""Regression：``TianShuOpenClawBridge._resolve_agent_for_request``。
...
不实例化真实 ``TianShuOpenClawBridge``（避免拉 blade/gymnasium）；用
...
def _resolve(self_like: SimpleNamespace, context: dict[str, Any] | None) -> Any:
    return TianShuOpenClawBridge._resolve_agent_for_request(self_like, context)
